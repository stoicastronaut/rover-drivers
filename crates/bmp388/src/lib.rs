...
//! Async, `no_std` I2C driver for the Bosch BMP388 barometric pressure sensor.
//!
//! The driver owns an [`embedded_hal_async::i2c::I2c`] bus handle and borrows
//! an async delay only for operations that need timing. [`Bmp388::init`] checks
//! the device identity, resets the sensor, loads its factory calibration, and
//! applies the selected [`Config`]. Measurements are returned in degrees
//! Celsius and pascals.
//!
//! ```no_run
//! use bmp388::{Address, Bmp388, Config};
//! use embedded_hal_async::{delay::DelayNs, i2c::I2c};
//!
//! async fn read_once<I2C, D>(i2c: I2C, delay: &mut D)
//!     -> Result<(), bmp388::Error<I2C::Error>>
//! where
//!     I2C: I2c,
//!     D: DelayNs,
//! {
//!     let mut sensor = Bmp388::with_config(i2c, Address::Primary, Config::default());
//!     sensor.init(delay).await?;
//!     let sample = sensor.measure_forced(delay).await?;
//!     let _temperature_c = sample.temperature_celsius;
//!     let _pressure_pa = sample.pressure_pa;
//!     Ok(())
//! }
//! ```
...
mod tests {
    ...
    const CALIBRATION_BYTES: [u8; 21] = [
        0x0A, 0x6B, 0xB4, 0x49, 0xF6, 0x0C, 0xFF, 0x4A, 0xF3, 0x23, 0x00, 0x17, 0x65, 0xF5, 0x7A,
        0xF3, 0xF6, 0xD6, 0x3F, 0x1D, 0xC4,
    ];
